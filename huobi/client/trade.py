...
class TradeClient(object):
    ...
    def get_etp_holding_limit(self, currencies: 'str') -> list:
        check_currency(currencies)

        params = {
            "currency": currencies
        }

        from huobi.service.trade.get_etp_holding_limit import GetEtpHoldingLimitService
        return GetEtpHoldingLimitService(params).request(**self.__kwargs)
